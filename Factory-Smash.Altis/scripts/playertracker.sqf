...
_vehicles = (position player) nearEntities [["Air"], 400];
	...
	if(((_x != player) && (player distance _x < 300)) || ((side _x isEqualTo side player) && (_x != player))) then {
		...
		_distSize = (1/(1+((1.9^-4)*_distFloor))) * 2 + 0.5;
		_profileName =  name _x;
		_alphaMod = 1/(1+((3^-3)*_distBetween)); 
		_friendAlpha = (_alphaMod * 4);
		if (_friendAlpha > 0.75) then {_friendAlpha = 0.75};
		_friendMod = 1;
		if (side _x isEqualTo side player) then [{_friendMod = 0},{_friendMod = 1}];
		...
		then
			...
			if (side _x isEqualTo side player) then {
				...
				{
					drawIcon3D ["\A3\ui_f\data\map\Markers\Military\dot_ca.paa", [_friendMod, _redMod, _yelMod, _friendAlpha], [(visiblePosition _x) select 0, (visiblePosition _x) select 1, _position select 2], _distSize, _distSize, 1,"",_distSize,0.02,"puristaMedium","Center",True];
				};
			}
				else {
				_objVis = lineIntersects [eyePos player, eyePos _x,player, _x];
