...
"areaops" setMarkerPos (getPos Radio);
	if !(NearIntel) then {
			...
			"respawn_west" setMarkerPos (getPos west_respawn_move);
			"respawn_east" setMarkerPos (getPos east_respawn_move);
			"respawn_guerrila" setMarkerPos (getPos indfor_respawn_move);
				};
	} else {
