...
addMissionEventHandler ["Draw3D", {
    {
        ...
        _unit = _x;
        {
            private["_positions","_color","_muzzleVelocity","_size","_objVis","_terVis"];
            ...
            for "_i" from 0 to (count _positions) - 2 do {
				...
				_objVis = lineIntersects [eyePos player, _positions select _i select 0,player];
				_terVis = terrainIntersect [visiblePosition player, _positions select _i select 0];
...
_unit = [_this, 0, player, [objNull]] call BIS_fnc_param;
    ...
    if (faction  _unit == "OPF_F") then {
        [ _unit, [1,0,0,0.7], 0, 3, 500, 1.5] call hyp_fnc_traceFire; 
    };
    if (faction  _unit == "BLU_F") then {
        [ _unit, [0,0,1,0.7], 0, 3, 500, 1.5] call hyp_fnc_traceFire; 
    };
    if (faction  _unit in ["IND_F", "IND_G_F", "CIV_F"]) then {
        [ _unit, [0,1,0,0.7], 0, 3, 500, 1.5] call hyp_fnc_traceFire; 
    };
